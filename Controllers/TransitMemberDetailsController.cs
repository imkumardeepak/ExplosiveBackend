...
namespace Peso_Baseed_Barcode_Printing_System_API.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class TransitMemberDetailsController : ControllerBase
    {
        private readonly ITransitMemberDetailsRepository _TransitMemberDetailsRepository;
        private readonly IDistributedCache _distributedCache;
        private readonly APIResponse _APIResponse;
        private readonly IMapper _mapper;
        private const string CacheKey = "TransMemberDetail";

        public TransitMemberDetailsController(ITransitMemberDetailsRepository TransitMemberDetailsRepository, IDistributedCache distributedCache, IMapper mapper)
        {
            _TransitMemberDetailsRepository = TransitMemberDetailsRepository;
        ...
        public async Task<ActionResult<APIResponse>> GetAllMembers()
        {
            try
            {
                ...
                {
                    var members = await _TransitMemberDetailsRepository.GetAllAsync();
        ...
        public async Task<ActionResult<APIResponse>> GetMemberById(int id)
        {
            ...
            try
            {
                var member = await _TransitMemberDetailsRepository.GetByIdAsync(id);
                if (member == null)
                {
                    _APIResponse.Status = false;
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<APIResponse>> CreateMember(TransitMemberDetails transMemberDetail)
        {
            try
            {
                ...
                await _TransitMemberDetailsRepository.AddAsync(transMemberDetail);
        ...
        public async Task<ActionResult<APIResponse>> UpdateMember(int id, TransMemberDetail transMemberDetail)
        {
            try
            {
                ...
                var existingMember = await _TransitMemberDetailsRepository.GetByIdAsync(id);
                ...
                await _TransitMemberDetailsRepository.UpdateAsync(existingMember);
        ...
        public async Task<ActionResult<APIResponse>> DeleteMember(int id)
        {
            try
            {
                ...
                await _distributedCache.RemoveAsync(CacheKey);
                await _TransitMemberDetailsRepository.DeleteAsync(id);
